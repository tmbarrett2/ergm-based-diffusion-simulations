...
# Tyler Barrett and Jonathan H. Morgan, Ph.D.
# 10 April 2024
...
# Set Working Directory
  setwd("/workspace/ergm-based-diffusion-simulations/ERGM_diagnostics_demonstration/simulated_mada_nets")
...
# Functions to Load Network Data
  
  # Function to Subset Indian Villages Network Data
  ...
  # Function to Extract Network by Tie Type
...
#   Loading the Networks
    synthetic_networks_files <- list.files(pattern = "\\.rds$")  # only .rds files
    synthetic_networks_list <- vector("list", length(synthetic_networks_files))
    names(synthetic_networks_list) <- synthetic_networks_files

    for (i in seq_along(synthetic_networks_files)) {
    file <- synthetic_networks_files[[i]]
    message("Reading: ", file)
    tryCatch({
        synthetic_networks_list[[i]] <- readRDS(file)
    }, error = function(e) {
        warning("Failed to read ", file, ": ", conditionMessage(e))
    })
    }

# Indian Villages Test Networks
  # Import Data
    directory <- getwd()
    load(paste0(directory,'/villageData/village_networks_2023/village_data_20July2023.Rda'))
    
  # Set Test Networks
    test_cases <- c(83, 43, 58)
    network_data <- village_subestting(test_cases, village_data)
    
  # Extract Tie-Type Specific Networks
    test_networks <- vector(mode = "list", length(test_cases))
    names(test_networks) <- test_cases
    for(network in seq_along(test_cases)){
      
      # Extract Networks
        free_time <- network_extractor(test_cases[[network]], 5)
        lending <- network_extractor(test_cases[[network]], 2)
        borrowing <- network_extractor(test_cases[[network]], 1)
        health_receiving <- network_extractor(test_cases[[network]], 3)
        health_giving <- network_extractor(test_cases[[network]], 4)
      
      # Store Networks in List
        test_networks[[network]] <- list(
          free_time = free_time,
          lending = lending,
          borrowing = borrowing,
          health_receiving = health_receiving,
          health_giving = health_giving
        )
    }
    
  # Assign Node Attributes
    test_network_names <- names(test_networks)
    for(i in seq_along(test_networks)){
      for(j in seq_along(test_networks[[i]])){
        attributes <- network_data$nodelist[(network_data$nodelist$villagecode == test_network_names[[i]]), ]
        ...
        test_networks[[i]][[j]] %v% "male_hh" <- attributes$oldest_male
        test_networks[[i]][[j]] %v% "kids" <- attributes$num_kid
        test_networks[[i]][[j]] %v% "house_caste" <- attributes$house_caste
        test_networks[[i]][[j]] %v% "house_edu" <- attributes$house_edu
      }
    }
...
# Concordance Check
# Provides a List of Dataframes with Concordance Values for Each Pair of Tie Types in Each Meta Network
  concordance_check_test <- ergm_concordance_check_iterative(test_networks)

# Degree Correlation Check
# Provides a List of Dataframes with Degree Correlations for Each Pair of Tie Types in Each Meta Network
# Provides a List of Correlation Plots Visualizing Degree Correlations for Each Pair of Tie Types in Each Meta Network
  correlation_check_test <- ergm_correlation_check_iterative(test_networks, 0.6)
    
# Attribute Check
# Provides Proportion of Nodes for Each Category and a Pareto Plot
  ergm_attributes_check_test <-  ergm_attributes_check_iterative(test_networks)
