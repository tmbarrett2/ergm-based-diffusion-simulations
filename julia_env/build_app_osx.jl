...
#   Determine CPU target for Mac
    cpu_target = "generic"
    if Sys.isapple()
        if Sys.ARCH == :aarch64  # Apple Silicon (M1/M2/M3)
            cpu_target = "apple-m1"
            @info "Detected Apple Silicon - using apple-m1 target"
        else  # Intel Mac
            cpu_target = "generic"
            @info "Detected Intel Mac - using generic target"
        end
    end
